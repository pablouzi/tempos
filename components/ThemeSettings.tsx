    ...
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-900 h-full shadow-2xl p-6 overflow-y-auto animate-fade-in flex flex-col border-l dark:border-gray-800 transition-colors duration-300">
        ...
        <div className="space-y-8 flex-grow">
          ...
          {/* 1. Presets Rápidos */}
          <section>
            <h3 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Estilos de Marca</h3>
            <div className="grid grid-cols-3 gap-2">
              {/* Preset CaféPOS / Libre Coffee */}
              <button
                onClick={() => applyPreset('coffee')}
                className="p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-[#F7F1E9] hover:border-[#C8553D] transition-colors flex flex-col items-center gap-1"
              >
                <div className="w-6 h-6 rounded-full bg-[#C8553D]"></div>
                <span className="text-xs font-medium text-gray-600">Libre Café</span>
              </button>
              ...
              >
                ...
                <span className="text-xs font-medium text-gray-600">Dopamina</span>
              </button>

              {/* Preset Dark / Noche */}
              <button
                onClick={() => applyPreset('dark')}
                className="p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-900 hover:border-blue-500 transition-colors flex flex-col items-center gap-1"
              >
          ...
          {/* 2. Color Picker (Ajustado a tu nueva paleta) */}
          <section>
            <h3 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Color de Acento</h3>
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
              {/* El naranja terracota ahora es el primero */}
              {['#C8553D', '#8d6e63', '#7c3aed', '#059669', '#2563eb', '#1F1F1F'].map(color => (
                <button
                  key={color}
                  onClick={() => updateSettings({ primaryColor: color })}
                  className={`w-10 h-10 rounded-full flex-shrink-0 transition-transform shadow-sm ${settings.primaryColor === color ? 'scale-110 ring-2 ring-offset-2 ring-orange-400' : ''}`}
